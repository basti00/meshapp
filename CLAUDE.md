...
* **Frames archive**: every received *non-message* frame is also stored verbatim in the `frames` table (`telemetry` / `position` / `nodeinfo` / `other`), one row per packet, with the decoded typed sub-payload in `payload_json` for `json_extract()` queries. Messages stay in their own table so frames can be FIFO-pruned later without touching chat history. `node_detail_api` materialises the newest frame of each kind into `node.latest` blocks (device and environment metrics separately — they arrive in different telemetry frames), so the node modal can show per-block ages and click through to the exact source frame (`/api/frames/<id>`). Pre-v11 databases are backfilled once at startup (`_migrate_v11_backfill_frames`, one snapshot frame per data kind per node, `raw_json` stamped `migrated: true`) — one-off code to remove after deploy.
...
* **Frontend**: Jinja templates render the initial page with data embedded as JSON; `static/app.js` handles theme and the node/message/channel/frame detail modals (fetching `/api/...` detail endpoints). Modals are created on demand and *stack* (`pushModal`/`popModal`): opening a detail from within a modal pushes a layer while the one beneath dims and recedes; the X pops one layer, background click closes the whole stack. The node modal's message/frame lists lazy-load via `<rx_time>:<id>` keyset cursors (`/api/nodes/<id>/messages|frames`). Scroll-back pagination and the live-update polling loop live inline in `templates/messages.html`. Avatar colors are derived deterministically from the node id (`_node_avatar_colors`).
